...
export class Cell {
  ...
  board: Board;
  available: boolean; // чи є можливісь фігурі переміщатися на цю ячейку
  ...
  moveFigure(target: Cell, boardState: BoardState): string {
    if (!this.figure) {
      return '';
    }
    ...
    // якщо ен пасант
    if (boardState.lastMoveIsEnPassant === true && target.isEnPassantFigure && boardState.lastCell) {
      const lastCell = this.board.getCell(boardState.lastCell?.x, boardState.lastCell?.y);

      if (lastCell.figure) {
        // видаляємо пішака який перестрибнув одну клітинку ( це поле lastCell в redux)
        lastCell.addLostFigure();
        lastCell.figure = null;
      }

      target.setFigure(this.figure);
      this.figure = null;
      return getPruningCoordinateMove(boardState, this, target, selectedFigure, opponentFigure);
    }
  ...
  getCellInfo(): CellInfo {
    return {
      x: this.x,
      y: this.y,
      figure: this.figure ? this.figure.nameEn : null,
      color:  this.figure ? this.figure.color : null,
    };
  }
}
