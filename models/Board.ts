...
export class Board {
    ...
    public initCells() {
        for (let i = 0; i < this.numberOfCells; i++) {
            ...
            for (let j = 0; j < this.numberOfCells; j++) {
                if ((i + j) % 2 !== 0) {
                    row.push(new Cell(this, j, i, PlayerTypes.BLACK, null))
                } else {
                    row.push(new Cell(this, j, i, PlayerTypes.WHITE, null))
                }
            }
            this.cells.push(row);
        }
    ...
    public highlightCells(selectedCell: Cell | null, currentPlayer: Player, boardState: BoardState) {
        for (let i = 0; i < this.cells.length; i++) {
            const row = this.cells[i];
            for (let j = 0; j < row.length; j++) {
                const target = row[j];
                if (selectedCell === null || selectedCell.figure === null) {
                    target.available = false;
                } else {
                    target.available = this.playerCanMove(selectedCell, target, currentPlayer);
                }

                if (boardState.lastMoveIsEnPassant && boardState.lastCell && boardState.prevCell) {
                    target.enPassantFigure = boardState.prevCell.x === target.x
                        && (target.y === (boardState.lastCell.y + boardState.prevCell.y) / 2);
                }
            }
        }
    }
    ...
    public castleMove(isBlack: boolean, isLongCastle: boolean): void {
        ...
        const cellOldRoot = this.getCellByCoordinate(isBlack ?
        ...
        const cellRoot = this.getCellByCoordinate(isBlack ?
        ...
        if (cellOldRoot.figure) {
            cellOldRoot.figure.moveFigure(cellRoot);
            cellRoot.setFigure(cellOldRoot.figure);
            cellOldRoot.figure = null;
        }
    ...
    playerCanMove(selectedCell: Cell, target: Cell, currentPlayer: Player) {
        ...
        return this.figureCanMove(selectedCell, target, currentPlayer);
    }

    figureCanMove(selectedCell: Cell, target: Cell, currentPlayer: Player) {
